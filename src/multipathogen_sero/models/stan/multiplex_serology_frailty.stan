functions{
    real smoothed_individual_hazard(
        real baseline_hazard,
        vector relevant_betas,
...
data { // TODO: code the censoring times in a smarter way
    ...
    real <lower=0> log_frailty_scale_scale; // scale for log frailty scale
    real <lower=1> log_frailty_corr_shape; // shape parameter for LKJ prior on log frailty correlation
}
...
model {
    ...
    log_frailty_scale ~ normal(0, log_frailty_scale_scale);
